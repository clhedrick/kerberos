...
// We first look at all processes in /proc. Check their environment
// for KRB5CCNAME. Put all of those cache names in a hash table.
// That makes the hash table have all the caches in use.
//
// We then check all caches listed in /run/renewdccs. Those are all
// the caches we know exist. If any isn't in the hash table, it's
// no longer active, and we kill it. Otherwise we renew it.

// Hash table entries have the cache name, the UID owning the cache,
// a flag indicating that it was generated by cron (actually by
// pam_kgetcred) and a link. The links are used to put all the
// entries into a single linked list. That lets us free them. Surprisingly,
// the hash library doesn't have a function to free the hash table and
// all of its entries, so we have to keep track of the entries.
...
renew(krb5_context ctx, krb5_ccache ccache, time_t minleft, int getcred, uid_t uid, char *ccname) {
    ...
    // here to renew a credential cache that can't be done with kgetcred.
    // this is basically "kinit -R"
    code = krb5_get_renewed_creds(ctx, &creds, user, ccache, NULL);
    creds_valid = 1;
    if (code != 0) {
      // renew failed. probably the ticket was expired
    ...
    // on top of original. that avoids any possible race
    // conditions between us and code using the cache. No
    // obvious way to do this for KEYRING.
...
// this is used primarily to read /proc/NNN/environ,
// but could be used to get any file
...
void getccs() {
  ...
  // look at /proc, looking for /proc/NNN
  // get a list of all files in /proc and go through them
  ...
  for (i = 0; i < numdirs; i++) {
    ...
    // only look at entries for procs, i.e. /proc/NNN
    if (first >= '0' && first <= '9') {
      char *fname;
      char *line;
      char *ptr;
	...
	free(line);
      ...
      // look for environment variable KRB5CCNAME, which is the
      // credential cache being used
	...
	ptr = ptr + strlen(ptr) +1;
      ...
      // didn't find anything. presumably this process isn't using kerberos
      if (ptr >= (line + fsize)) {
	free(line);
	free(namelist[i]);
	continue;
